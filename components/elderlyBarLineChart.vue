    ...
    const option = {
        ...
        tooltip: {
            ...
            formatter: params => {
                ...
                return `
                    <div style="background-color: #124776; padding: ${16 * scale}px; border-radius: 0; border: ${
    2 * scale
}px solid rgba(158, 202, 255, 0.4); box-shadow: 0 ${6 * scale}px ${24 * scale}px ${2 * scale}px rgba(3, 28, 45, 0.8);">
                        <h4 style="font-family: MicrosoftYaHei; font-size: ${28 * scale}px; color: #FFFFFF; font-weight: 700;">${tooltipTitle}</h4>
                        <div style="display: grid; grid-auto-rows: ${37 * scale}px; grid-row-gap: ${8 * scale}px; grid-template-columns: ${
    32 * scale
}px min-content min-content; grid-column-gap: ${4 * scale}px ${12 * scale}px; align-items: center; margin-top: ${8 * scale}px;">
                            ${params
                                ...
                                .map(n => {
                                    ...
                                    const svgPath = legendIconMap[n.seriesType || 'bar']
                                        .replace('path://', '')
                                        .replace(/(?<!(a|A)(-?\d+(\.\d+)?,){3,4})-?\d+(\.\d+)?/g, s => s * scale)
                                    return `
                                        <i style="height: ${32 * scale}px; position: relative; overflow: hidden;">
                                            <svg viewbox="0 0 ${32 * scale} ${32 * scale}" width="${32 * scale}" height="${
    32 * scale
}" style="filter: drop-shadow(${32 * scale}px 0 0 ${color}); margin-left: ${
    -32 * scale
}px; position: absolute; top: 0; left: 0;">
                                                <path d="${svgPath}"></path>
                                            </svg>
                                        </i>
                                        <label style="white-space: nowrap; font-family: MicrosoftYaHei; font-size: ${
    28 * scale
}px; color: #FFFFFF; font-weight: 400; grid-column-start: 3; margin-left: ${6 * scale}px; display: ${
    n.seriesName ? 'block' : 'none'
};">${n.seriesName}</label>
                                        <label style="white-space: nowrap; font-family: MicrosoftYaHei; font-size: ${
    28 * scale
}px; color: #FFFFFF; font-weight: 400; grid-column-start: 5; margin-left: ${6 * scale}px;">${
    [null, undefined, '', NaN].includes(n.value) ? '- -' : n.value
}${unit}</label>
                                    `
                                })
                                .join('')}
        ...
        series: (() => {
            ...
            const series = props.seriesData.map((seriesItem, seriesIndex) => {
                ...
                if (seriesOption.type === 'bar') {
                    seriesOption.barWidth = 48 * scale
                ...
                if (seriesOption.type === 'line') {
                    ...
                    seriesOption.symbolSize = 14 * scale
                    ...
                    (seriesOption.markLine = {
                        ...
                        data: markLines.map(n => ({
                            ...
                            label: {
                                ...
                                padding: [6 + 6, 20, 0 + 6].map(n => n * scale),
                                offset: [-38, 0].map(n => n * scale),
